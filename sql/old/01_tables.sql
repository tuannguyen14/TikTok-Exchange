...
CREATE TABLE public.profiles (
    ...
    tiktok_stats JSONB DEFAULT '{}', -- followers, following, total_videos, etc.
...
-- Bảng lưu video đã bookmark/save
...
-- Bảng theo dõi người dùng (following/followers)
CREATE TABLE public.user_follows (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    follower_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE,
    following_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE,
    -- Lưu TikTok username để track follow trên TikTok
    following_tiktok_username VARCHAR(100),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(follower_id, following_id),
    CHECK (follower_id != following_id)
);

-- Bảng video đã xem trong exchange
