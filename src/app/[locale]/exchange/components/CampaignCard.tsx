...
export default function CampaignCard({
    ...
    // Get initial video info for like actions
    useEffect(() => {
        if (campaign.campaign_type === 'video' &&
            campaign.interaction_type === 'like' &&
            campaign.tiktok_video_id &&
            campaign.target_tiktok_username) {

            exchange.getInitialVideoInfo(campaign).then(response => {
                if (response.success) {
                    setInitialVideoInfo(response.data);
                }
            });
        }
    }, [campaign]);
    ...
    const getDisplayInfo = () => {
        if (campaign.campaign_type === 'video' && campaign.video_info) {
            return {
                ...
                subtitle: `${exchange.formatCount(campaign.video_info.playCount)} ${t('stats.views')}`,
                avatar: campaign.video_info.url,
                stats: [
                    {
                        icon: <IconHeart size={16} />,
                        label: t('stats.likes'),
                        value: exchange.formatCount(campaign.video_info.diggCount)
                    },
                    {
                        icon: <IconMessageCircle size={16} />,
                        label: t('stats.comments'),
                        value: exchange.formatCount(campaign.video_info.commentCount)
                    }
                ]
            };
        } else if (campaign.campaign_type === 'follow' && campaign.user_info) {
            return {
                title: campaign.user_info.nickname,
                subtitle: `@${campaign.user_info.uniqueId}`,
                avatar: campaign.user_info.avatarThumb,
                stats: [
                    {
                        icon: <IconUsers size={16} />,
                        label: t('stats.followers'),
                        value: exchange.formatCount(campaign.user_info.followerCount)
                    },
                    {
                        icon: <IconUsers size={16} />,
                        label: t('stats.following'),
                        value: exchange.formatCount(campaign.user_info.followingCount)
                    }
                ]
            };
        }
            ...
            >
                ...
                {/* <Group justify="center" mb="md">
                    <Badge
                        variant="light"
                        size="lg"
                        color={campaign.campaign_type === 'video' ? 'blue' : 'green'}
                        style={{ fontWeight: 600 }}
                    >
                        {campaign.campaign_type === 'video' ? t('campaignTypes.video') : t('campaignTypes.follow')}
                    </Badge>
                </Group> */}
